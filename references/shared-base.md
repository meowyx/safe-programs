# Shared Base - Solana Security Rules
# Applies to every Solana program regardless of framework (Native Rust, Anchor, or Pinocchio).
...
Most exploited surface in Solana. Every account your program receives is untrusted until you check it.
...
PDAs hold all your program's state. A bad PDA is a permanent backdoor.
...
Same account passed twice for two different roles. Classic, and easy to miss.
...
CPI is the deepest surface in Solana. Every CPI is a trust boundary.
...
Security isn't a checklist. It's an adversarial mindset applied while you're designing, not after.
...
Reward math is the most exploited surface in staking and yield protocols. Every pattern below maps to a validated Critical or High finding from real audits.
...
### 24.1 Lockup Must Be Enforced on Every Reward Claim Path
...
## 32. Instruction Introspection & Self-CPI Patterns

Solana programs can read the current transaction's instruction list via `sysvar::instructions`. This enables flash-loan detection, batching intent checks, and cross-instruction coupling. It also creates a class of bugs that look like reentrancy on other chains. The rules below cover using introspection safely and not trusting transactions to enforce security.

### 32.1 Verify the Instructions Sysvar Address
- The `sysvar::instructions` account is user-supplied like any other account. If you don't verify it, an attacker passes a fake account and spoofs the entire instruction list back to your program.
- Anchor: use `Sysvar<'info, Instructions>` or the explicit account address check. The typed sysvar enforces the address.
- Native / Pinocchio: verify explicitly before reading.
...
### 32.2 Instruction Presence Is Not Instruction Effect
- Seeing a specific instruction in the transaction does not prove it did what you think it did. An attacker can call the real handler with arguments that make it a no-op: `deposit_usdc(amount = 0)`, `update_oracle(price = stale_price)`, `vote(weight = 0)`. Your check sees the instruction; no effect happened.
- Checking only the program ID is insufficient. Verify program ID **and** the instruction's discriminator, and pair both with the expected account set. Different programs can emit bytes that superficially look like yours.
- Anchor uses an 8-byte SHA-256-based discriminator. Native / Pinocchio programs use whatever tag they define. Know the schema of whichever program you're inspecting.
- Use introspection for **coarse detection** (flash loan present, bundled-intent check), not for fine-grained authorization. If you need to authorize based on another instruction having run, design the authorization as explicit state (set a flag in the other handler, read it here), not as "this instruction also appears in the tx."

### 32.3 Flash Loan Detection - Partial at Best
- Naive: reject if the preceding top-level instruction is from a flash-loan program. Bypassable - an attacker wraps their flash loan in another program's handler that CPIs into the flash-loan program. Your check never sees the wrapped call.
- Less naive: walk every top-level instruction and reject matches against a known flash-loan list.

```rust
// WRONG - only checks the instruction immediately before this one
let prev = instructions::load_instruction_at_checked(
    current_index.saturating_sub(1),
    sysvar_account,
)?;
require!(prev.program_id != FLASH_LOAN_PROGRAM_ID, ErrorCode::FlashLoanDetected);
```

```rust
// LESS NAIVE - walk every top-level instruction
let mut i: usize = 0;
loop {
    match instructions::load_instruction_at_checked(i, sysvar_account) {
        Ok(ix) => {
            require!(
                !FLASH_LOAN_PROGRAMS.contains(&ix.program_id),
                ErrorCode::FlashLoanDetected
            );
            i = i.checked_add(1).ok_or(ErrorCode::Arithmetic)?;
        }
        Err(_) => break,
    }
}
...
- Even walking the full top-level list misses **nested** flash loans - an attacker's program CPIs into the flash-loan program, and that nested call never appears at the top level. `sysvar::instructions` exposes top-level only.
- **The honest industry standard: do not rely on introspection as the primary defense.** Architectural defenses are stronger:
  - **Time-weighted balances** - positions must be held N slots/blocks before they count toward voting, rewards, or collateral value
  - **Snapshot at action time** - vote weight, collateral value, reward eligibility captured when the position was opened, not read live
  - **Minimum hold periods** - deposits can't withdraw in the same transaction, so a flash loan can't round-trip through your protocol
  - **TWAP oracles** - price reads average over a window, not current spot
- Use introspection-based detection as belt-and-suspenders, not the main defense. If protocol safety depends on catching flash loans via introspection, the design needs to change - the attack is eventually inevitable.
- Allow-list vs deny-list tradeoff: deny-list of known flash-loan programs goes stale (new flash-loan programs appear); allow-list of known-safe callers rejects legitimate users of unknown programs. Pick based on risk tier.

### 32.4 Self-CPI and Transfer Hook Reentry
- Solana's runtime blocks classic EVM reentrancy at the account level - the same account can't hold two mutable borrows at once, so a re-entry that borrows the same account fails.
- But a program can still be re-entered through:
  - **Token-2022 transfer hooks** that CPI into arbitrary program code during a transfer (most common vector)
  - **Self-CPI** where your program calls itself with a different account set
  - **Nested CPI chains** where a downstream program CPIs back up to you
- Failure mode is the same as EVM reentrancy: external call happens before state commits, re-entry reads stale state, double-acts.
- Mitigation: **checks-effects-interactions** - commit all state changes (balances, `reward_debt`, flags) **before** any external CPI that could re-enter. Combine with `reload()` after CPIs (see §5.2).

```rust
// WRONG - transfer first, state update second. A transfer hook can re-enter
// claim_rewards and see the pre-update reward_debt.
token_interface::transfer_checked(cpi_ctx, amount, decimals)?;
position.reward_debt = new_debt;

// RIGHT - state first, transfer second
position.reward_debt = new_debt;
token_interface::transfer_checked(cpi_ctx, amount, decimals)?;
```

- Token-2022 transfer hooks run arbitrary program code during a transfer. Treat every `transfer_checked` with a Token-2022 mint as a potential re-entry point. Combine this rule with §23 (extension validation at init).
- For long-running handlers, add an explicit reentrancy-lock flag on the state account:

```rust
require!(!state.in_progress, ErrorCode::Reentrant);
state.in_progress = true;
// ... work, including CPIs ...
state.in_progress = false;
```

### 32.5 Cross-Instruction State Assumptions
- Handler B that assumes handler A ran first in the same transaction is broken if B is callable standalone. Enforce ordering via explicit state flags on the account (`state.step == Step::AfterDeposit`), not via transaction composition.
- See §26.4 (allowlist-based transition guards) and §26.7 (terminal states must be absorbing) for the full state-machine pattern. The rule here is a specific application: the state machine is the authoritative ordering, the transaction is not.

### 32.6 Top-Level vs CPI-Invoked Distinction (Situational)
- Most programs don't need this. Apply selectively to handlers that should never be reachable through another program's CPI - admin-only configuration changes, emergency pause triggers, upgrade acceptance.
- Stack height reveals CPI depth. Top-level instructions run at stack height 1. Any deeper is a CPI context.
...
require!(
    get_stack_height() == 1,
    ErrorCode::MustBeTopLevelInstruction
);
```

- If the handler **should** be CPIable by specific trusted callers, use a program-ID allowlist of trusted parents instead of a blanket depth check.
