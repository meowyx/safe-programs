...
- **Capacity constraints.** For every `require!(value <= limit)`, find every path that increases `value`. Identify paths that skip the check.
...
- **Bypass cap enforcement.** Enumerate every path modifying a capped value - deposits, fee accrual, admin operations, emergency mode. Find the path that skips the check.
