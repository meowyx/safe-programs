...
For each vector, extract the root cause and hunt every manifestation - different names, account types, structures. A "stale cached account data" vector applies wherever code caches cross-program state or reads an account before CPI and uses it after.
...
A guard only stops you if it blocks every path. Find the way around:
